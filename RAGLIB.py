...
def bot2(messages):
    ...
    payload = {
        ...
        "frequency_penalty": 1.6,
        "max_tokens": 1600, # Adjust as needed
        "stream": False, # <--- CHANGED: Set stream to False
        "stop": STOPS
        # Add other parameters your Llama.cpp server supports, e.g.:
        # "n_predict": 1000, # llama.cpp specific equivalent to max_tokens
        # "top_k": 40,
    ...
    # Create a session to reuse connections if you're making multiple calls
    session = requests.Session() 
    ...
    try:
        response = session.post(
            f"{BASE_URL}/chat/completions",
            json=payload,
            # stream=True # <--- REMOVED: No longer streaming
        )
        response.raise_for_status()  # Raise an HTTPError for bad responses (4XX or 5XX)

        # <--- CHANGED: Handle non-streaming response
        response_json = response.json()
        
        assistant_response_content = ""
        if response_json.get("choices") and len(response_json["choices"]) > 0:
            # For non-streaming, the 'message' key directly contains the full response
            message = response_json["choices"][0].get("message", {})
            content = message.get("content")
            if content:
                assistant_response_content = content
                print(content) # Print the full content once received
        ...
        if assistant_response_content: # Only add to history if content was received
            ...
            # Handle cases where no content is returned but no error occurred
            return {"role": "assistant", "content": "No content received from AI."}
